...
object ShoconPlugin extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[_]] = Seq(
    ...
    shoconFilter := { _: (String, InputStream) =>
      true
    },
  ...
  private def loadConfigs(
    ...
    ((if (loadFromJars) loadDepReferenceConfigs(dependecyClassPath, log)
      else Nil) ++ loadProjectConfigs(unmanagedResources, log)).filter(fileFilter)
  ...
  private def loadDirReferenceConfigs(dirs: Seq[File], log: Logger): Seq[(String, InputStream)] = {
    val files = dirs.map(_ / "reference.conf").filter(_.isFile).map(f => (f.getAbsolutePath, fin(f)))
    log.debug(
      "SHOCON config files found in project dependencies:\n" + files.map("    " + _._1).mkString("", "\n", "\n\n"))
    files
  }
