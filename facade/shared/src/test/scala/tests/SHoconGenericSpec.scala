...
object SHoconGenericSpec extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("parseStringLiteralsWithSlashes") {
    ...
    test("parseLists") {
      val config1 = ConfigFactory.parseString("""l =[ a
          |
          |   b
          |  c
          |
    ...
    test("parseNewLinesInsteadOfCommas") {
    ...
    test("parseDurations") {
      ...
      assert { config.getDuration("a.x").toMillis == 1L }
      assert { config.getDuration("a.x", TimeUnit.NANOSECONDS) == 1000000L }
    }
    ...
    test("dottedConfigKeyWithFallback") {
      val configAkka =
        ConfigFactory
    ...
    test("reparseKey") {
      ...
      val expected = shocon.Config.Object(
        Map("foo" -> shocon.Config.Object(
          Map("bar" -> shocon.Config.Object(Map("baz" -> shocon.Config.StringLiteral("quux")))))))

      assert { expected == reparsed }
    }

    test("mergeConfigValues") {
      ...
      val expected = shocon.Config.Object(
        Map("foo" -> shocon.Config.Object(
          Map("bar" -> shocon.Config.Object(Map("baz" -> StringLiteral("quux"), "bazz" -> StringLiteral("quuxxx")))))))
    ...
    test("concatValues") {
      val x = ConfigFactory.parseString("""x="foo"
          |y=   z "bar" """.stripMargin)
      assert { "foo" == x.getString("x") }
      assert { "z bar" == x.getString("y") }
    }

    test("properlyFallback") {
    ...
    test("parseListOfObjectsWithNewLineSeparatedObjects") {
      // protect against having this parsed at compile time
      var x = ""
      val conf = ConfigFactory.parseString(s"""
        $x
    ...
    test("parseListOfObjectsWithTrailingCommas") {
      // protect against having this parsed at compile time
      var x = ""
      val conf = ConfigFactory.parseString(s"""
        $x
