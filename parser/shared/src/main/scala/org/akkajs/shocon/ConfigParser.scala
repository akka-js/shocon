...
object ConfigParser {
  ...
  val isWhitespaceNoNl: Char => Boolean = (c: Char) => c != '\n' && isWhitespace(c)
  ...
  def string[_: P] =
    P(nlspace) ~ P(quotedString | unquotedString | CharsWhile(_.isSpaceChar).!) // bit of an hack: this would parse whitespace to the end of line
      .rep(1)
      .map(_.mkString.trim) // so we will trim the remaining right-side
      .map(Config.StringLiteral)

  // *** Parsing ***
  def array[_: P]: P[Seq[Config.Value]] =
    P("[" ~ nlspace ~/ jsonExpr.rep(sep = itemSeparator) ~ nlspace ~ ",".? ~ nlspace ~ "]")
  ...
  def pair[_: P]: P[(String, Config.Value)] =
    P(string.map(_.value) ~/ space ~ ((keyValueSeparator ~/ jsonExpr) | (repeatedObj ~ space)))
  ...
  def objBody[_: P] = P(pair.rep(sep = itemSeparator) ~ nlspace)

  def jsonExpr[_: P] = P(space ~ (repeatedObj | repeatedArray | string) ~ space)

  def root[_: P] = P((&(space ~ "{") ~/ obj) | (objBody) ~ End).map(x => Config.Object.fromPairs(x))
